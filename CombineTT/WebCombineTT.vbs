'update 20170609 : update formate 
Option Explicit 
...
'dbHost = "10.252.163.98"      ' Hostname of the database server
'dbPort = "1521"      ' Hostname of the database server
'dbName = "TTSUAT"           ' Name of the database/SID
'dbUser = "tts"               ' Name of the user
'dbPass = "tts"               ' Password of the above-named user
...
'IP|User|Date Update|C+AlertID|Severity|Description|Misc|YYYY|AMO|CombindTT|FAULT|Site|Col1|TT_FromWeb|Old TT_ID|AlertName|system_value
'IP|User|Date Update|C+AlertID|Severity|Description|Misc|Location|CombindTT|FAULT|Site|Col1|TT_FromWeb|AlertName|system_value
'IP|UserName|alDate|alId|Severity|Problem|Brand|Location|Title|eventType|siteCode|bsc|TT_ID|AlertName|system_value
...
Set AlertStream = AlertShell.CreateTextFile(AlertFile,true)
	...
	AlertStream.WriteLine "<WebCombineTTWindows>"
	AlertStream.WriteLine "CombineTTWindows|COMBINE|" _
	& GetIPAddresses & "|"_
	& AlertFormat(0) & "|" _
	& Split(WScript.Arguments(0),"|")(2) & "|" _
	& "C" & AlertFormat(1) & "|" _
	& AlertFormat(4) & "|" _ 
	& AlertFormat(6) & "|" _ 
	& "Misc|" _
	...
	& "ZZZZ|" _ 
	& AlertFormat(3) & "|" _
	& systemValue(WScript.Arguments(0))
	AlertStream.WriteLine AlertFormat(0) & "</WebCombineTTWindows>"
	AlertStream.close
...
function systemValue(txt)
	If InStr(txt,"3G2100")  Then
		systemValue = "32"
	Elseif InStr(txt,"4G1800") Then
		systemValue = "64"
	Elseif InStr(txt,"4G") Then
		systemValue = "64"
	Elseif InStr(txt,"4G2100") Then
		systemValue = "64"
	Elseif InStr(txt,"2G900") Then
		systemValue = "8"
	Elseif InStr(txt,"GSM900") Then
		systemValue = "8"
	Elseif InStr(txt,"3G900") Then
		systemValue = "16"
	Else
		systemValue = "xxx"
	end if
...
Function DateConversion(parDate)
	...
	Dim Curr_Year : Curr_Year = (Mid(Year(Date),1,2) & str_yy)
	...
	DateConversion = str_dd  & "/" & str_mm  & "/" & Curr_Year & " " & str_time	
End Function
